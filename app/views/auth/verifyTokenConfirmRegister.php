...
<div class="container rounded mt-5" style="background:white;max-width:500px; position:relative; z-index:2;">
    <div class="row rounded shadow pb-4">
          ...
          <input type="hidden" name="idUserTokenAccount" value="<?=$user['idUser']?>">
          <h2 class="text-center py-2">
              ...
              <div class="shadow-sm p-3 mb-3 bg-body-tertiary rounded" style="color:var(--bs-secondary-color);">
                <?php
                if(isset($_SESSION['successMailAccount'])):
               ?>
                <div class="alert alert-success alert-dismissible fade show" role="alert">
                   <strong>
                     <?=$_SESSION['successMailAccount']??''?>
                   </strong>
              ...
              ?>
               <?php
                if($user!==null):
              ?>
              <h4 class="">¡Bienvenid@ <?=$user['name']?> se a confirmado tu correo. Te hemos enviado un token para activar tu cuenta <strong><?=$user['mail']?></strong> de forma segura!</h4>
              <h5 class="fw-light">"Por favor, verifica tu bandeja de entrada, incluida la carpeta de correo no deseado."</h5>
            <?php
              endif;
            ?>
            <?php
                if(isset($_SESSION['errorTokenAccount'])):
              ?>
              <div class="alert alert-danger alert-dismissible fade show" role="alert">
                 <strong>
                   <?=$_SESSION['errorTokenAccount']??''?>
                 </strong>
          ...
          <button class="w-100 btn-lg btn background-general" type="submit" style="" name="btnVerifyTokenConfirmRegister"><span class="fs-5">Confirmar Token</span>
          </button>
          <?php
            if(isset($_SESSION['stateToken']) AND $_SESSION['stateToken'] == 0):
          ?>
            <button class="w-100 btn-lg btn background-general" type="submit" style="" name="btnResetTokenConfirmRegister"><span class="fs-5">Reenviar Token</span>
            </button>
          <?php
            endif;
          ?>
